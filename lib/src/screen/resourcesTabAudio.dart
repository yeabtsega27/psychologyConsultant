...
class resourcesTabAudio extends StatefulWidget {
  String search;

  resourcesTabAudio({super.key,
    required this.search,
  });

  @override
  State<resourcesTabAudio> createState() => _resourcesTabAudioState();
}

class _resourcesTabAudioState extends State<resourcesTabAudio> {
  ...
  Widget build(BuildContext context) {
    getResourcesList() async {
      ...
      var response = await http.post(Uri.parse(apiUrl), body: {
        'query': "SELECT * FROM `resource` WHERE `type`='audio' AND `name` like '%${widget.search}%'"
      });
                    ...
                    child: Row(
                      children: [
                        SizedBox(
                          width: 130,
                          child: videoPlayer(
                            Url: discussionData[index]['resource_url'],
                          ),
                        ),
                        Expanded(
                            child: Padding(
