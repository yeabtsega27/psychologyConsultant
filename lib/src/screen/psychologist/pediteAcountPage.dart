...
class pediteAccountPage extends StatefulWidget {
  const pediteAccountPage({Key? key}) : super(key: key);

  @override
  State<pediteAccountPage> createState() => _pediteAccountPageState();
}

class _pediteAccountPageState extends State<pediteAccountPage> {
  ...
  uploadFile() async {
    ...
    try {
      var response = await http.post(Uri.parse(apiCon.apiPsyProfile),
  ...
  getUserInfo() async {
    ...
    var respons = await http.post(Uri.parse(apiCon.apiMysqlQuery), body: {
      'query':
      "SELECT psychology.*, USER.email FROM `psychology` psychology INNER JOIN `user` AS USER ON psychology.psychology_id = USER.user_id AND psychology.psychology_id =$userId"
    });
  ...
  saveUserInfo() async {
    ...
    var respons = await http.post(Uri.parse(apiCon.apiMysqlInsert), body: {
      'query':
      "UPDATE `psychology` SET `first_name`='${_firstNameTextControler.text}',`last_name`='${_lastNameTextControler.text}',`phone`='${_phoneNameTextControler.text}',`gender`='${genderSelectedValue}' WHERE `psychology_id`=$userId"
    });
    if (respons.statusCode == 200) {
      var jsonData = json.decode(respons.body);
      if (jsonData['status'].toString().contains("error")) {
